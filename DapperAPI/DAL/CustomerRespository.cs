...
namespace DapperAPI.DAL
{
    ...
    {
        IDbConnection _db = new SqlConnection(
        ...
        {
            return _db.Query<Customer>("SELECT TOP " + amount + " [CustomerID],[CustomerFirstName],[CustomerLastName],[IsActive] FROM [Customer] ORDER BY CustomerID " + sort).ToList();
        }
